...
(function ($) {
	$.extend({
		...
		operate: {
			...
			// 删除已读信息
			removeRead: function(id,status) {
				if(status==='0'){
					$.modal.alertWarning("该消息未读");
				}else {
					$.modal.confirm("确定删除该条" + $.table._option.modalName + "信息吗？", function() {
						var url = $.common.isEmpty(id) ? $.table._option.removeUrl : $.table._option.removeUrl.replace("{id}", id);
						if($.table._option.type == table_type.bootstrapTreeTable) {
			...
			//标记已读
			editRead:function(id){
				var url = $.common.isEmpty(id) ? $.table._option.editReadUrl : $.table._option.editReadUrl.replace("{id}", id);
				$.operate.submit(url, "get", "json");
			},
			readMsg:function(id){
				var urls = $.common.isEmpty(id) ? $.table._option.readMsgUrl : $.table._option.readMsgUrl.replace("{id}", id);
				$.ajax({
					type: 'get',
					url: urls,
					timeout: 1500,
					...
					success: function (data) {
						console.log(data)
						$.modal.confirm(data.data.content, function() {});
					}
				})
